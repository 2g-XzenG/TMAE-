...
def cost_loss_fun(y_true, y_pred):
    y_true = tf.reshape(y_true, [-1])
    y_pred = tf.reshape(y_pred, [-1])
    loss = tf.cast(tf.abs(y_true - y_pred), tf.float32)
    mask = tf.cast(tf.not_equal(y_true, 0), tf.float32)
    loss = tf.multiply(loss, mask)
    return tf.reduce_sum(loss)/tf.reduce_sum(mask)
        
def code_loss_fun(y_true, y_pred):
...
def model(
    ...
    ccs_seq = layers.Input(shape=(max_visit, max_code), name="ccs_feature")
    util_seq = layers.Input(shape=(max_visit), name="util_feature")
    date_seq = layers.Input(shape=(max_visit), name="date_feature")
    cost_seq = layers.Input(shape=(max_visit), name="cost_feature")

    inputs = [demo, code_seq, ccs_seq, util_seq, date_seq, cost_seq]
    ...
    # ccs sequence
    ccs_mask = layers.Lambda(create_code_mask)(ccs_seq)
    ccs_emb = layers.Embedding(input_dim=ccs_vocab, 
                               output_dim=vocab_dim, 
                               weights=[ccs_embed_matrix], 
                               trainable=True,
                               name='ccs_embed')(ccs_seq)
    ccs_emb = layers.Multiply()([ccs_emb, ccs_mask])
    ccs_emb = tf.reduce_sum(ccs_emb, axis=2)     
    
    code_emb = tf.concat([ccs_emb, code_emb], axis=-1)
    code_emb = layers.Dense(vocab_dim, activation=None)(code_emb)
    ...
    # cost sequence    
    cost_emb = layers.Embedding(input_dim=cost_vocab, output_dim=vocab_dim, mask_zero=True, name='cost_embedding')(cost_seq)
    cost_emb = layers.Multiply()([cost_emb, visit_mask[:,:,tf.newaxis]])

    # visit sequence
    visit_emb = layers.Add()([code_emb, date_emb, util_emb, cost_emb]) 
    ...
    demo_emb = tf.expand_dims(patient_embedding, 1) # (N, 1, emb_size)    
    demo_emb = layers.Dense(vocab_dim, activation=None)(demo_emb)
    
    query_emb = tf.concat([date_emb, util_emb], 2)
    query_emb = layers.Dense(vocab_dim, activation=None)(query_emb)

    multihead = multihead_attention(model_dim, num_heads)(
        queries=query_emb,
        keys=tf.concat([demo_emb, query_emb], 1),
        values=tf.concat([demo_emb, query_emb], 1),
        query_masks=visit_mask,
        key_masks=tf.concat([demo_mask, visit_mask], 1)) # (N, 2*max_visit, emb_size)
    visit_emb = ffn(model_dim, ffn_dim)(multihead)
    
    # code_label = layers.Dense(units=model_dim, activation=tf.nn.sigmoid)(visit_emb)
    code_label = layers.Dense(units=code_vocab, activation=tf.nn.sigmoid, name='code_label')(visit_emb)
    
    cost_label = layers.Dense(units=model_dim, activation=tf.nn.relu)(visit_emb)
    cost_label = layers.Dense(units=1, activation=tf.nn.relu, name='cost_label')(cost_label)
    
    outputs = [code_label, cost_label]
